...
def process_args():
    ...
    parser.add_argument('--tool-diameter', action="store", dest='tool_diameter',                        help="Tool diameter associated with cut.")
...
def create_shellfile(filename, tool_diameter):                                  
    ...
    filename = PATH to file to convert. Supports gbr, tcl, and svg.
    tool_diameter = The diameter of the given cutting tool, IN INCHES.
    
    output from function -> tcl file naamed "create_gcode.tcl"
    output when run in flatcam -> gcode file named "current_job.gcode"
    """
    ...
    if (filename[len(filename)-3 : len(filename)] == "svg"):
        #TODO Implement SVG file support.
        ...
        # TODO Implement DRL file support.
    ...
    elif (filename[len(filename)-4 : len(filename)] == "gcode"):
        # TODO Implement pure GCODE support. Would be good to visualize via flatcam.
        pass 
    else:
        print("Filetype not supported. Executing most recent shellfile.")
    tcl_file.close()  
